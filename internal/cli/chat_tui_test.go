...
// the alt buffer with cell-motion mouse reporting so wheel events reach the
// transcript viewport, and the frame is exactly the terminal height (the
...
func TestViewAltScreenFillsHeight(t *testing.T) {
	...
	if v.MouseMode != tea.MouseModeCellMotion {
		t.Error("View must enable cell-motion mouse reporting so transcript wheel scrolling reaches the viewport")
	}
